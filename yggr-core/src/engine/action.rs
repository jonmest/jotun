...
pub enum Action<C> {
    ...
    ///
    /// `membership` is the cluster membership as of
    /// `last_included_index` — the host stores it alongside the bytes
    /// so membership survives restart and snapshot-based catch-up.
    ///
